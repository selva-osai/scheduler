...
//						container.get(0).options=this.options;
	...
	jQuery.fn.containerSetIcon = function (icon,path){
		...
		if (icon && icon!="" ){
			...
			//container.find(".n:first").css({paddingLeft:10});
		}else{
	...
	jQuery.fn.containerSetButtons = function (buttons,opt){
		...
		if (buttons !=""){
			...
			for (var i in btn){
				if (btn[i]=="m"){
					container.find(".buttonBar:first").append("<img src='"+path+opt.skin+"/max.png' class='collapsedContainer'/>");
					container.find(".n:first").bind("dblclick",function(){container.mb_fullscreen(opt);});
					btn[i] = 'r';
					container.find(".collapsedContainer:first").bind("click",function(){container.mb_fullscreen(opt);
						var foundElement = container.find(".collapsedContainer:first");
						foundElement.hide();
						container.find(".restoreContainer:first").show();
						container.find(".restoreContainer:first").removeAttr('src','images/max.png');
						container.find(".restoreContainer:first").attr('src','images/restore.png');
					});			
				}
				...
				if (btn[i]=="r"){
					container.find(".buttonBar:first").append("<img src='"+path+opt.skin+"/min.png' class='restoreContainer'/>");
					container.find(".restoreContainer:first").hide();
					container.find(".restoreContainer:first").bind("click",function(){
						container.mb_resizeTo(300,700);
						container.mb_switchFixedPosition();
						container.find(".collapsedContainer:first").removeAttr('src','images/min.png');
						container.find(".collapsedContainer:first").attr('src','images/max.png');						
						var foundElement = container.find(".restoreContainer:first");
						foundElement.hide();
						container.find(".collapsedContainer:first").show();
					});
				}
			}
	...
	jQuery.fn.mb_fullscreen= function(){
		...
		if (container.mb_getState('iconized') || container.mb_getState('collapsed') || container.mb_getState('closed')){
			...
			container.attr("h",$(window).height()-140);
			container.attr("t",20);
			container.attr("l",20);
			container.css("height","");
			return;
		...
		container.mb_resizeTo($(window).height()-140,$(window).width()-40);
